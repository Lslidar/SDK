...
std::vector<MuchLidarData> GetLidarData::getLidarPerFrameDate() {
    isFrameOK = false;
    return LidarPerFrameDate;
}


#pragma region //set the parameters of the lidar, send the device package 
...
bool GetLidarData::setLidarIP(std::string LidarIPValue) {
    ...
    if (setLidarParam()) {
        ...
        IP_result.clear();
        LidarIPValue = LidarIPValue + ".";                                                     //easily obtain the last section of data
        for (size_t i = 0; i < LidarIPValue.size(); i++)                                     //cut out "defaultIP lineedit" content
        {
...
//check whether the setting of Lidar IP is correct
...
//check whether the destination IP is correct
...
bool GetLidarData::setComputerIP(std::string ComputerIPValue) {
    ...
    if (setLidarParam()) {
        ...
        for (size_t i = 0; i < ComputerIPValue.size(); i++)                                     //obtain destIP lineedit
        {
...
bool GetLidarData::setLidarParam() {
    ...
    if (true == islidarDevCome && true == isSendUDP) {
        ...
        //Save the device package before sent the configuration package
