...
#define ConversionAngle_C_v4_0        20.25                //the unit of the angle conversion between optical center and structural center is degree
...
void GetLidarData_C1_v4_0::LidarRun() {
    while (true) {
        ...
        if (!allDataValue.empty()) {
            ...
            timestamp_nsce = (data[1209] * pow(2, 24) + data[1208] * pow(2, 16) + data[1207] * pow(2, 8) +
                              data[1206] * pow(2, 0));
    ...
    if (data[0] == 0xff && data[1] == 0xee) {
        ...
        for (int i = 0; i < 12; i++) {
                ...
                for (int j = 0; j < 32; j++) {
                    ...
                    if (m_DataT[j].ID == 6) {
                        ...
                        if (all_BlockAngle.size() > 100) {
                            if ((m_DataT[j].H_angle - endFrameAngle > 0) &&
                                abs(m_DataT[j].H_angle - endFrameAngle) < 20) {
            ...
            for (int j = 0; j < 32; j++) {
                m_DataT[j].ID = j % 8;
                m_DataT[j].Distance =
                        (data[5 + 3 * j + 100 * i] * 256 + data[4 + 3 * j + 100 * i]) * IDistanceACC_C_v4_0;
    ...
    if (data[0] == 0xff && data[1] == 0xee) {
        ...
        for (int i = 0; i < 12; i += 2) {
            ...
            for (int j = 0; j < 32; j++) {
                ...
                m_DataT_d[j].ID = j % 8;
                m_DataT_d[j].Distance =
                        (data[5 + 3 * j + 100 * (i + 1)] * 256 + data[4 + 3 * j + 100 * (i + 1)]) * IDistanceACC_C_v4_0;
...
bool GetLidarData_C1_v4_0::setLidarRotateState(int RotateState) {
    if (setLidarParam()) {
...
bool GetLidarData_C1_v4_0::setLidarSoureSelection(int StateValue) {
    if (setLidarParam()) {
        if (StateValue != 0 && StateValue != 1 && StateValue != 2) {
            std::string str = "StateValue can only be equal to 0 or 1 or 2, please check the input parameters";
            messFunction(str, 0);
            return false;
        }
...
bool GetLidarData_C1_v4_0::setLidarWorkState(int LidarState) {
    if (setLidarParam()) {
